...
{  
  ...
  nalu->startcodeprefix_len=3;//初始化码流序列的开始字符为3个字节  
   ...
   memcpy (Buf,h264Buf + cursor,3);//从码流中读3个字节  .fwrite(oinfo.StrmVirAddr,1,oinfo.dataSize,fp_strm);
//   printf ("BUF: %c%c%c\n",Buf[0],Buf[1],Buf[2]);  
   cursor+=3;
...
//		printf ("开始前缀为4个字节");  
   ...
   //查找下一个开始字符的标志位  ，因为我每次传的都必定是一个，是不是不需要这里了
  ...
  {  
    if (cursor == (h264BufSize))//判断是否到了文件尾  
    {  
      ...
      free(Buf);  
//	  printf("GetAnnexbNALU 到达文件末尾，:%d ,%d",pos,cursor);
      return pos;  
    }  
...
//  printf("找到了 :pos:%d,cursor:%d ,startcodeprefix_len:%d\n",pos,cursor,nalu->startcodeprefix_len);
  ...
  /*)
  if (0 != fseek (bits, rewind, SEEK_CUR))//把文件指针指向前一个NALU的末尾  SEEK_CUR： 当前位置
  {  
    free(Buf);  
...
//	   memcpy(h264Buf+oinfo.headerSize+oinfo.dataSize,oinfo.StrmVirAddr,4);
	...
	//printf("新增图像");
	if(seq_num ==0){
		if ((h264Buf = (unsigned char*)calloc (oinfo.dataSize+oinfo.headerSize , sizeof(char))) == NULL)   
		printf ("新增图像: Could not allocate Buf memory\n");  
		h264BufSize = oinfo.dataSize+oinfo.headerSize;
		memcpy(h264Buf,h264HeadBuf,oinfo.headerSize);
		memcpy(h264Buf+oinfo.headerSize,oinfo.StrmVirAddr,oinfo.dataSize);
	//	fwrite(h264Buf,1,h264BufSize,fp_strm2);
	}else{
	if ((h264Buf = (unsigned char*)calloc (oinfo.dataSize , sizeof(char))) == NULL)   
		printf ("新增图像: Could not allocate Buf memory\n");  
		h264BufSize = oinfo.dataSize;
		//memcpy(h264Buf,h264HeadBuf,oinfo.headerSize);
		memcpy(h264Buf,oinfo.StrmVirAddr,oinfo.dataSize);
	//	fwrite(h264Buf,1,h264BufSize,fp_strm2);
	}
	//   fclose(fp_strm2);
    ...
    while(cursor < (h264BufSize))  //直到文件结束，改进的话应该是传来的数据发完 
    {  
		//--第一处修改
		//printf("到达GetAnnexbNALU");
        GetAnnexbNALU(n,oinfo);//每执行一次，文件的指针指向本次找到的NALU的末尾，下一个位置即为下个NALU的起始码0x000001  
